  ...
  return (
    <div className="container" style={{ padding: '2rem 0' }}>
      ...
      <div style={{
        ...
        <div>
          <span style={{ fontSize: '0.9rem', color: 'var(--primary)', fontWeight: 600 }}>{t('executorPortal')}</span>
          <h2 style={{ fontSize: '2rem', marginTop: '0.25rem' }}>{t('myDesk')}</h2>
        </div>
        ...
        {!currentUser && (
          <div style={{
            ...
            <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('roleExecutor')}:</span>
            <select 
                ...
                <div key={task.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.5rem' }}>
                    <div>
                      ...
                      <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t('clientLabel')}: {task.company}</span>
                    </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="card" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 600 }}>{t('currentBalance')}</span>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', width: '100%', padding: '0.75rem 0', justifyContent: 'center' }}>
              <div>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.9rem', color: '#fbbf24', fontWeight: 700 }}>
                  <Star size={14} fill="#fbbf24" style={{ stroke: '#fbbf24' }} /> {activeExecutor.rating}
                </span>
                <span style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{t('rating')}</span>
              </div>
              <div style={{ width: '1px', background: 'var(--border-color)' }}></div>
              <div>
                <span style={{ fontSize: '0.9rem', fontWeight: 700 }}>{activeExecutor.completedTasks}</span>
                <br />
                <span style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{t('taskCountText')}</span>
              </div>
            </div>

            <div style={{ width: '100%', textAlign: 'left' }}>
              <h4 style={{ fontSize: '0.85rem', marginBottom: '0.5rem', color: 'var(--text-main)' }}>{t('skills')}</h4>
